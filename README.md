...
**`cdfgg`** это утилита для генерации статических и динамических CFG/DFG-графов по LLVM IR из исходного кода. Проект встраивает LLVM pass plugin в `clang`, сохраняет IR до и после оптимизаций, при необходимости добавляет runtime-инструментацию, а затем превращает результат в сериализуемый `ir_graph` JSON, `dot` и `svg`.

В отличие от обычного дампера, `cdfgg` ориентирован не только на визуализацию, но и на дальнейший анализ графа. Поэтому центральным артефактом здесь является не `dot`, а независимое промежуточное представление `ir_graph`, которое потом:

- сериализуется в JSON,
- мерджится с runtime-профилем,
- конвертируется в `dot`,
- рендерится в `svg`.
...
| `--program-arg ARG`                                | аргумент запускаемой инструментированной программы                   |
| `--program-args ...`                               | все оставшиеся аргументы передаются программе                        |
...
## Сравнение графа до оптимизаций и после
...
Проект является учебным. Выполнен студентом 2 курса ФРКТ МФТИ в рамках допкурса по LLVM от [Сергея Лисицына](https://github.com/lisitsynSA)
